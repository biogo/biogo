// From algorithm by Deborah Toledo Flores
//
...
LOOP:
	for position := start; position+length < end; position++ {
		...
		scores = append(scores, &feat.Feature{
			...
			End:        position + length,
			Score:      score,
			Attributes: []byte(string(sequence.Seq[position:position+length]) + " " + strconv.Ftoa64(prob, self.FloatFormat, self.Precision)),
