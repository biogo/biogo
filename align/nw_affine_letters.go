...
func (a NWAffine) alignLetters(rSeq, qSeq alphabet.Letters, alpha alphabet.Alphabet) ([]feat.Pair, error) {
	...
	table[1] = [3]int{
		diag: minInt,
		up:   minInt,
		left: a.GapOpen + a.Matrix[gap][index[qSeq[0]]],
	}
	for j := range table[2:c] {
		table[j+2] = [3]int{
			diag: minInt,
			up:   minInt,
			left: table[j+1][left] + a.Matrix[gap][index[qSeq[j+1]]],
		}
	}
	table[c] = [3]int{
		diag: minInt,
		up:   a.GapOpen + a.Matrix[index[rSeq[0]]][gap],
		left: minInt,
	}
	for i := 2; i < r; i++ {
		table[i*c] = [3]int{
			diag: minInt,
			up:   table[(i-1)*c][up] + a.Matrix[index[rSeq[i-1]]][gap],
			left: minInt,
		}
	}
