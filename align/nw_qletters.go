...
func drawNWTableQLetters(rSeq, qSeq alphabet.QLetters, index alphabet.Index, table []int, a [][]int) {
	...
	for i := 0; i < r; i++ {
		...
		for j := 0; j < c; j++ {
			pr := pointerNWRuneQLetters(rSeq, qSeq, i, j, table, index, a, c)
			if pr != ' ' {
...
func pointerNWRuneQLetters(rSeq, qSeq alphabet.QLetters, i, j int, table []int, index alphabet.Index, a [][]int, c int) rune {
	switch {
	case i == 0 && j == 0:
		return ' '
	case i == 0:
...
func (a NW) alignQLetters(rSeq, qSeq alphabet.QLetters, alpha alphabet.Alphabet) ([]feat.Pair, error) {
	...
	for j := range table[1:c] {
		table[j+1] = table[j] + a[gap][index[qSeq[j].L]]
	}
	for i := 1; i < r; i++ {
		table[i*c] = table[(i-1)*c] + a[index[rSeq[i-1].L]][gap]
	}
